...
function valueOfType(type: 'string'|'number'|'object') {
  return (x: unknown) => assert(typeof x == type)
}

const localIp = (x: unknown) => assert(typeof x == 'string' && ['::1', '127.0.0.1'].includes(x))
